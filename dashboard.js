...
function loadCards() {
    ...
    let filteredCards = cards.filter(c => !filter || c.user === filter);
    filteredCards.sort((a, b) => a.user.localeCompare(b.user));
...
async function exportarPDF() {
    ...
    // Ordenar também no PDF
    cards.sort((a, b) => a.user.localeCompare(b.user));

    doc.setFontSize(18);
    doc.text("Relatório de Manutenções", 10, y);
    y += 10;
    ...
    cards.forEach(card => {
        ...
        doc.text(`Custo: ${formatarCusto(Number(card.custo))}`, 10, y); y += 7; // custo formatado
